...
import problem1.item.display.LCDPanel;
import problem1.item.processor.RaspberryPi;

public class DeluxeSystem implements SystemBuilder{
    private QMSystem qmSystem = new QMSystem();
    @Override
    public void addDisplayUnit(int count) {
        for(int i=0;i<count;i++){
            qmSystem.addItem(new RaspberryPi());
            qmSystem.addItem(new LCDPanel());
        }
    }
